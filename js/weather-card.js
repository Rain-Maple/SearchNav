...
class WeatherCard extends HTMLElement {
  ...
  _initTemplate() {
    this.shadowRoot.innerHTML = `
            <style>
                ...
                .city-name {
                    font-weight: 600;
                    background: rgba(255, 255, 245, 0.69);
                    padding: 4px 12px;
                    border-radius: 30px;
                    color: #1f3a5f;
  ...
  _initEventListeners() {
    ...
    this.$closeBtn.addEventListener("touchstart", closeHandler, {
      passive: false,
    });
  ...
  _initDrag() {
    ...
    if (this.$dragHeader) {
      ...
      this.$dragHeader.addEventListener("touchstart", startDrag, {
        passive: false,
      });
    }
  }
  ...
  async _loadWeatherData(lat, lon, showLoading = true) {
    if (showLoading)
      this.$content.innerHTML = '<div class="loading">🔄 刷新数据中...</div>';
    if (this.abortController) this.abortController.abort();
    ...
    try {
      ...
      if (retryBtn)
        retryBtn.onclick = () => this._loadWeatherData(lat, lon, true);
    } finally {
  ...
  _getWeatherInfo(code) {
    const map = {
      ...
      81: { icon: "☔💧", desc: "强阵雨" },
      82: { icon: "💧💧", desc: "猛烈阵雨" },
      85: { icon: "❄️🌨️", desc: "小雪阵" },
      86: { icon: "❄️🌨️🌨️", desc: "大雪阵" },
      95: { icon: "⛈️", desc: "雷暴" },
  ...
  _renderFullUI(current, hourly, daily, lat, lon) {
    ...
    const pressure = current.pressure_msl
      ...
      : null;
    const usAqi =
      current.us_aqi !== undefined ? Math.round(current.us_aqi) : null;
    ...
    const infoColumnHtml = `
            <div class="info-column">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value">${pressure !== null ? pressure + " 百帕" : "—"}</span>
                </div>
                <div class="info-row">
      ...
      if (resp.ok) {
        ...
        if (data && data.status === "0" && data.result) {
          ...
          if (cityName && cityName !== "undefinedundefined") {
            cityName =
              cityName.length > 30 ? cityName.slice(0, 28) + "…" : cityName;
